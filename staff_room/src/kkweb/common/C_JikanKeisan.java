...
public class C_JikanKeisan {
	...
	public String cyokusetuJikan(String start, String end,String code,String cyoku_work,String cyouka_work,String pcode, B_Code ctbl){
		...
		try{
			...
			if(code.substring(0,1).equals("f") || code.substring(0,1).equals("F")){
...
//			String sql = "select * from codeMST where PROJECTcode ='"+pcode+"' AND KINMUcode ='"+code+"'";
...
//			ctbl = (B_Code)clist.get(0);
			...
			int kihon = ((endtime_h*60)+endtime_m)-((starttime_h*60)+starttime_m);
			int cyokusetu;
			...
			}else if(work_today > 480 && end_h*60+end_m > endtime_h*60+endtime_m){
	...
	public String cyoukaJikan(String end,String pcode,String code,String cyouka_work,String start, B_Code ctbl){

		try{

			if(end.equals("") || pcode.equals("") || code.equals("") || start.equals("")){
			...
			if(code.equals("96")){
				...
				if(endjikan > 2200){
					...
					cyouka = ((end_h*60)+end_m)-((start_h*60)+start_m);
...
//			int kihon = ((endtime_h*60)+endtime_m)-((starttime_h*60)+starttime_m);
	...
	public String sinyaJikan(String start,String end,String code,String sinya_work){
		...
		try{
			...
			if(Integer.parseInt(start_hm) > 500 && Integer.parseInt(start_hm) <= 2200 && Integer.parseInt(end_hm) > 2200 && Integer.parseInt(end_hm) <= 2900){
				end_h = Integer.parseInt(end.substring(0,2));
				end_m = Integer.parseInt(end.substring(2));
			...
			}else if(Integer.parseInt(start_hm) > 500 && Integer.parseInt(start_hm) <= 2200 && Integer.parseInt(end_hm) > 2200 && Integer.parseInt(end_hm) > 2900){

				sinyajikan = "700";

			}else if(Integer.parseInt(start_hm) < 500 && Integer.parseInt(end_hm) <= 2200){
				start_h = Integer.parseInt(start.substring(0,2));
				start_m = Integer.parseInt(start.substring(2));
			...
			}else if(Integer.parseInt(start_hm) < 500 && Integer.parseInt(end_hm) > 2200 && Integer.parseInt(end_hm) <= 2900){
				start_h = Integer.parseInt(start.substring(0,2));
				start_m = Integer.parseInt(start.substring(2));
				end_h = Integer.parseInt(end.substring(0,2));
				end_m = Integer.parseInt(end.substring(2));

				sinya = ((5*60)-((start_h*60)+start_m))+(((end_h*60)+end_m)-(22*60));
				sinya_h = sinya/60;
				sinya_m = sinya%60;
			...
			}else if(Integer.parseInt(start_hm) >= 2200 && Integer.parseInt(end_hm) <= 2900){
				start_h = Integer.parseInt(start.substring(0,2));
				start_m = Integer.parseInt(start.substring(2));
				end_h = Integer.parseInt(end.substring(0,2));
				end_m = Integer.parseInt(end.substring(2));
	...
	public String furoujikan(String start,String end,String pcode,String code,String furou_work, B_Code ctbl){

		try{
			if(start.equals("") || end.equals("") || pcode.equals("") || code.equals("")){
			...
			if(code.equals("96") || /*code.equals("97") ||*/ code.equals("98") || code.equals("99")){
...
//			ctbl = (B_Code)clist.get(0);
			...
			//修正箇所
				...
				if(s_w > s_k){
					furou_s = (start_h*60+start_m) - (starttime_h*60+starttime_m) ;
				}
				if(e_w < e_k){
					furou_e = (endtime_h*60+endtime_m) - (end_h*60+end_m);
				}
	...
	public String restJikan(String start,String end, String code, String cyoku,String rest_work){
		...
		try{
			...
			int rest = (((end_h*60)+end_m)-((start_h*60)+start_m))-((cyoku_h*60)+cyoku_m);
	...
	public String checkPname(String pcode,String code){
		...
		try{
			...
			if(code.substring(0,1).equals("f") || code.substring(0,1).equals("F")){
				...
				CodeDAO cdao = new CodeDAO();
				String sql2 = " where projectcode ='"+pcode+"' AND KINMUcode ='"+code+"'";

				if(cdao.isThereTbl(sql2)){

					//sql = " select * from codeMST where projectcode ='"+pcode+"' AND KINMUcode ='"+code+"'";
		...
		//			return bcode.getPROJECTname()+shift_kinmu+"("+bcode.getBikou()+")";
					return shift_kinmu;
				}else{
	...
	public String deletePname(String pcode,String code,String pname){
		...
		try{
			...
			CodeDAO cdao = new CodeDAO();
			String sql = " where projectcode ='"+pcode+"' AND KINMUcode ='"+code+"'";
			...
			if(cdao.isThereTbl(sql)){
				...
				B_Code bcode = new B_Code();
				bcode = (B_Code)clist.get(0);
				String work = bcode.getPROJECTname()+shift_kinmu+"("+bcode.getBikou()+")";
