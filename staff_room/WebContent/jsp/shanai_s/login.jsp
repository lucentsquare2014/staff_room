...
<%@ page
	import="java.sql.*,java.io.*,java.util.*,java.util.Date,java.text.*"%>
...
<%
	...
	if (flag) {
		//スケジュールの削除
		ResultSet rs2 = stmt
				.executeQuery("SELECT * FROM PE_TABLE WHERE K_社員NO = '"
						+ ID + "'");
		while (rs2.next()) {
			...
			if (strSche != null) {
				...
				upDate.execute("DELETE FROM S_TABLE WHERE GO_社員NO = '"
						+ ID + "' AND S_DATE <= '" + sFmt.format(today)
						+ "'");
				upDate.execute("DELETE FROM B_TABLE WHERE K_社員NO = '"
						+ ID + "' AND B_END <= '" + sFmt.format(today)
						+ "'");
			}
		}
		rs2.close();
