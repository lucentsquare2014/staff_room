...
<%
	...
	//グループ
	ResultSet rs = stmt
			.executeQuery(" select * from GRU order by G_GRUNO");
	...
	rs = stmt
			.executeQuery("select * from KOJIN order by K_PASS2,K_社員NO");
	...
	if (table_select.equals("holiday")) {
		...
		for (int w = 0; w < a_count; w++) {
			sql = "insert into HOLIDAY values('" + shuku_day[w] + "','"
					+ shuku_name[w] + "')";
			upData.execute(sql);
		}
	}
	...
	if (table_select.equals("user")) {
		if (sql_select.equals("ins")) {
			...
			if (teishi_flg.equals("0")) {
				sql = "insert into KOJIN(K_社員No,K_承認LV,K_ID,K_PASS,K_氏名,K_GRUNo,K_MAIL) values('"
						+ k_no
						+ "','"
						+ k_lv
						+ "','"
			...
			if (teishi_flg.equals("0")) {
				sql = "update KOJIN set K_承認Lv='" + k_lv + "',K_ID='"
						+ k_id + "',K_PASS='" + k_pass + "',K_氏名='"
						+ k_name + "',K_GRUNo='" + k_gruno
	...
	if (table_select.equals("gru")) {
		if (sql_select.equals("ins")) {
			...
			if (teishi_flg.equals("0")) {
				sql = "insert into GRU values('" + g_code + "','"
						+ g_name + "')";
			}
		} else if (sql_select.equals("del")) {
			...
			if (teishi_flg.equals("0")) {
				sql = "update GRU set G_GRUNO='" + gr_num
						+ "',G_GRNAME='" + gr_name
