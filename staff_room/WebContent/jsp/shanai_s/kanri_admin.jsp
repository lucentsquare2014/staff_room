...
<%@ page
	import="java.sql.*,java.io.*,java.util.*,java.util.Date,java.text.*,java.lang.*"%>
...
<%
	...
	if (request.getParameter("table_select") != null
			&& !request.getParameter("table_select").equals("")) {
		table_select = request.getParameter("table_select");
	} else {
		table_select = "";
	}
	if (request.getParameter("sql_select") != null
			&& !request.getParameter("sql_select").equals("")) {
	...
	if (table_select.equals("holiday")) {
		...
		// 春分の日
		sup_day = Long.toString(Math.round(20.8431 + 0.242194
				* (intYear - 1980) - (intYear - 1980) / 4 - 0.5));
		shuku_day[a_count] = select_year + "03" + sup_day;
		shuku_name[a_count] = "春分の日";
		// 曜日を調べる
		cal = new GregorianCalendar(intYear, 2,
				Integer.parseInt(sup_day));
		if (cal.get(cal.DAY_OF_WEEK) == 1) {
			a_count++;
			shuku_day[a_count] = select_year + "03"
					+ Integer.toString(Integer.parseInt(sup_day) + 1);
			shuku_name[a_count] = "振替休日";
		}
		a_count++;
		...
		// 秋分の日
		sup_day = Long.toString(Math.round(23.2488 + 0.242194
				* (intYear - 1980) - (intYear - 1980) / 4 - 0.5));
		shuku_day[a_count] = select_year + "09" + sup_day;
		shuku_name[a_count] = "秋分の日";
		// 曜日を調べる
		cal = new GregorianCalendar(intYear, 8,
				Integer.parseInt(sup_day));
		if (cal.get(cal.DAY_OF_WEEK) == 1) {
			a_count++;
			shuku_day[a_count] = select_year + "09"
					+ Integer.toString(Integer.parseInt(sup_day) + 1);
			shuku_name[a_count] = "振替休日";
		}
		a_count++;
		...
		for (int w = 0; w < a_count; w++) {
			sql = "insert into KINMU.HOLIDAY values('" + shuku_day[w]
					+ "','" + shuku_name[w] + "')";
			upData.execute(sql);
		}
	}
	...
	if (table_select.equals("user")) {
		if (sql_select.equals("ins")) {
			String k_lv = "";
			if ((request.getParameter("ex1") == null || request
					.getParameter("ex1").equals(""))
					&& (request.getParameter("ex2") == null || request
							.getParameter("ex2").equals(""))) {
				k_lv = "0";
			...
			if ((request.getParameter("number") == null || request
					.getParameter("number").equals(""))
					|| (request.getParameter("usrid") == null || request
							.getParameter("usrid").equals(""))
					|| (request.getParameter("usrpw") == null || request
							.getParameter("usrpw").equals(""))
					|| (request.getParameter("usrname") == null || request
							.getParameter("usrname").equals(""))
					|| (request.getParameter("group") == null || request
							.getParameter("group").equals(""))
					|| (request.getParameter("mail") == null || request
							.getParameter("mail").equals(""))) {
				teishi_flg = "2";
			} else {
			...
			if (teishi_flg.equals("0")) {
				sql = "insert into KINMU.KOJIN(K_社員No,K_承認LV,K_ID,K_PASS,K_氏名,K_GRUNo,K_MAIL) values('"
						+ k_no
						+ "','"
						+ k_lv
						+ "','"
			...
			sql = "delete from KINMU.KOJIN where K_社員NO='" + k_no + "'";
			...
			if (request.getParameter("new_id") == null
					|| request.getParameter("new_id").equals("")) {
				teishi_flg = "7";
			} else if (request.getParameter("new_pw") == null
					|| request.getParameter("new_pw").equals("")) {
				teishi_flg = "8";
			} else if (request.getParameter("new_name") == null
					|| request.getParameter("new_name").equals("")) {
				teishi_flg = "10";
			} else if (request.getParameter("new_mail") == null
					|| request.getParameter("new_mail").equals("")) {
				teishi_flg = "9";
			} else {
			...
			if (teishi_flg.equals("0")) {
				sql = "update KINMU.KOJIN set K_承認Lv='" + k_lv
						+ "',K_ID='" + k_id + "',K_PASS='" + k_pass
						+ "',K_氏名='" + k_name + "',K_GRUNo='" + k_gruno
						+ "',K_MAIL='" + k_mail + "' where K_社員NO='"
						+ k_no + "'";
			}
		}
	}
	if (table_select.equals("gru")) {
		if (sql_select.equals("ins")) {
			...
			if ((request.getParameter("gr_code") == null || request
					.getParameter("gr_code").equals(""))
					|| request.getParameter("gr_name") == null
					|| request.getParameter("gr_name").equals("")) {
				teishi_flg = "2";
			} else {
			...
			if (teishi_flg.equals("0")) {
				sql = "insert into KINMU.GRU values('" + g_code + "','"
						+ g_name + "')";
			}
		} else if (sql_select.equals("del")) {
			String g_code = request.getParameter("gr_num");
			sql = "delete from KINMU.GRU where G_GRUNO='" + g_code
					+ "'";
		} else if (sql_select.equals("upd")) {
			String gr_num = "";
			String gr_name = "";

			if (request.getParameter("n_gr_name") == null
					|| request.getParameter("n_gr_name").equals("")) {
				teishi_flg = "13";
			} else if (request.getParameter("n_gr_num") == null
					|| request.getParameter("n_gr_num").equals("")) {
				teishi_flg = "14";
			} else {
			...
			if (teishi_flg.equals("0")) {
				sql = "update KINMU.GRU set G_GRUNO='" + gr_num
						+ "',G_GRNAME='" + gr_name
						+ "' where G_GRUNO='" + gr_num + "'";
			}
		}
	}
	...
	if (table_select.equals("yotei")) {
		if (sql_select.equals("ins")) {
			...
			if (request.getParameter("y_data") == null
					|| request.getParameter("y_data").equals("")) {
				teishi_flg = "2";
			} else {
			...
			if (teishi_flg.equals("0")) {
				sql = "insert into KINMU.yotei values('" + y_kubun
						+ "','" + y_naiyou + "','" + y_junban + "')";
			}
		} else if (sql_select.equals("del")) {
			String y_kubun = request.getParameter("y_kubun");
			String y_naiyou = strEncode(request.getParameter("y_basyo"));
			sql = "delete from KINMU.yotei where 区分='" + y_kubun
					+ "' and 場所='" + y_naiyou + "'";
		}
	}

	if (!table_select.equals("") && !table_select.equals("holiday")
			&& teishi_flg.equals("0")) {
		upData.execute(sql);
...
<body
	style="bgcolor: #F5F5F5; link: #000099; alink: 00FFFF; vlink: 000099">
	<form NAME="MyForm">
		<input type="button" style="background-color: #FFFFFF; width: 50"
			value="戻る"
			onClick="location.href='http://www.lucentsquare.co.jp/kinmu_db/top.html'">
	</form>
			...
			<table>
				<tr>
					<td><INPUT TYPE="button"
						STYLE="background-color: #FFFFFF; width: 100" VALUE="ユーザー一覧"
						onClick="location.href='#USER'"></td>
					<TD><INPUT TYPE="button"
						STYLE="background-color: #FFFFFF; width: 100" VALUE="グループ登録"
						onClick="location.href='#GROUP'"></TD>
					<td><INPUT TYPE="button"
						STYLE="background-color: #FFFFFF; width: 100" VALUE="予定区分登録"
						onClick="location.href='#YOTEI'"></TD>
					<TD><INPUT TYPE="hidden" NAME="action" VALUE="get_holiday">
						<INPUT TYPE="hidden" NAME="table_select" VALUE="holiday">
						<INPUT TYPE="hidden" NAME="sql_select" VALUE="ins"> <INPUT
						TYPE="hidden" NAME="pass" VALUE="<%=inpPassword%>"> <INPUT
						TYPE="submit" VALUE="祝日"
						STYLE="background-color: #FFFFFF; width: 100"> <SELECT
						NAME="year">
							<%
	...
	<TABLE STYLE="width: 100%">
		<TR>
			<TH bgcolor="#000099"><FONT color="#FFFFFF"><B>1.
						ユーザー登録</B></FONT></TH>
		</TR>
	</TABLE>
	...
	<CENTER>
		<TABLE>
			<TR>
				<TD><SMALL>1.管理者の方は責任を持って管理して下さい。</SMALL></TD>
			</TR>
			<TR>
				<TD><SMALL>2.承認する立場の人は、承認者の欄にチェックして下さい。</SMALL></TD>
			</TR>
		</TABLE>
		<BR>
		<BR>
		...
		<FORM action="kanri_admin.jsp" method="POST">
			<INPUT TYPE="hidden" NAME="action" VALUE="regist"> <INPUT
				TYPE="hidden" NAME="table_select" VALUE="user"> <INPUT
				TYPE="hidden" NAME="sql_select" VALUE="ins"> <INPUT
				TYPE="hidden" NAME="pass" VALUE="<%=inpPassword%>">
			<TABLE>
				<TR>
					<TD align="center">承認者</TD>
					<TD align="center" colspan="3">-１<INPUT TYPE="checkbox"
						NAME="ex1">- -２<INPUT TYPE="checkbox" NAME="ex2">-
					</TD>
				</TR>
				<TR>
					<TD align="center">Ｉ Ｄ</TD>
					<TD align="center"><INPUT TYPE="text" NAME="usrid" SIZE="20">
					</TD>
					<TD align="center">ﾒｰﾙｱﾄﾞﾚｽ</TD>
					<TD align="center"><INPUT TYPE="text" SIZE="30" NAME="mail">
					</TD>
				</TR>
				<TR>
					<TD align="center">ﾊﾟｽﾜｰﾄﾞ</TD>
					<TD align="center"><INPUT TYPE="text" NAME="usrpw" SIZE="20">
					</TD>
					<TD align="center">社員番号</TD>
					<TD align="left"><INPUT TYPE="text" SIZE="16" NAME="number"
						maxlength="20"></TD>
				</TR>
				<TR>
					<TD align="center">氏 名</TD>
					<TD align="center"><INPUT TYPE="text" NAME="usrname" SIZE="20">
					</TD>
				...
				<TR>
					<TD COLSPAN="4" align="center"><INPUT TYPE="submit"
						STYLE="background-color: #FFFFFF; width: 50" VALUE="   ＯＫ   ">
						<INPUT TYPE="reset" STYLE="background-color: #FFFFFF; width: 50"
						VALUE="Ｃｌｅａｒ"></TD>
				</TR>
			</TABLE>
		</FORM>
	...
	<CENTER>
		<TABLE STYLE="width: 100%">
			<TR>
				<TH bgcolor="#000099"><FONT color="#FFFFFF"><B>2.
							ユーザー一覧表 示</B></FONT></TH>
			</TR>
		</TABLE>
			...
			<TR>
				...
				<FORM action="kanri_henkou.jsp" method="POST">
					<TD><INPUT TYPE="hidden" NAME="action"
						VALUE="kojin_change_gamen"> <INPUT TYPE="hidden"
						NAME="table_select" VALUE="user"> <INPUT TYPE="hidden"
						NAME="sql_select" VALUE="upd"> <INPUT TYPE="hidden"
						NAME="number" VALUE="<%=kojin[0][y]%>"> <INPUT
						TYPE="hidden" NAME="pass" VALUE="<%=inpPassword%>"> <INPUT
						TYPE="submit" STYLE="background-color: #FFFFFF" VALUE="変更">
					</TD>
				</FORM>
				<FORM action="kanri_admin.jsp" method="POST">
					<TD NOWRAP><INPUT TYPE="hidden" NAME="action" VALUE="user_del">
						<INPUT TYPE="hidden" NAME="table_select" VALUE="user"> <INPUT
						TYPE="hidden" NAME="sql_select" VALUE="del"> <INPUT
						TYPE="hidden" NAME="pass" VALUE="<%=inpPassword%>"> <INPUT
						TYPE="hidden" NAME="user_del" value="<%=kojin[0][y]%>">
	...
	<CENTER>
		<TABLE STYLE="width: 100%">
			<TR>
				<TH bgcolor="#000099"><FONT color="#FFFFFF"><B>4.
							グループ登録</B></FONT></TH>
			</TR>
		</TABLE>
		...
		<FORM action="kanri_admin.jsp" method="POST">
			<TABLE>
				<TR>
					<TD>グループコード</TD>
					<TD><INPUT TYPE="text" NAME="gr_code" SIZE="10"></TD>
				</TR>
				<TR>
					<TD>グループ名</TD>
					<TD><INPUT TYPE=text NAME="gr_name" SIZE="30"></TD>
				</TR>
			</TABLE>
			<INPUT TYPE="hidden" NAME="action" VALUE="gr_input"> <INPUT
				TYPE="hidden" NAME="table_select" VALUE="gru"> <INPUT
				TYPE="hidden" NAME="sql_select" VALUE="ins"> <INPUT
				TYPE="hidden" NAME="pass" VALUE="<%=inpPassword%>"> <INPUT
				TYPE="submit" STYLE="background-color: #FFFFFF; width: 50"
				VALUE="登録">
		</FORM>
		<BR>
		<TABLE border="1">
			...
			<TR STYLE="font-size: 12">
				...
				<FORM action="kanri_henkou.jsp" method="POST">
					<TD><INPUT TYPE="hidden" NAME="gr_num"
						VALUE="<%=group[0][z]%>"> <INPUT TYPE="hidden"
						NAME="gr_name" VALUE="<%=group[1][z]%>"> <INPUT
						TYPE="hidden" NAME="action" VALUE="gr_change_gamen"> <INPUT
						TYPE="hidden" NAME="table_select" VALUE="gru"> <INPUT
						TYPE="hidden" NAME="sql_select" VALUE="upd"> <INPUT
						TYPE="hidden" NAME="pass" VALUE="<%=inpPassword%>"> <INPUT
						TYPE="submit" STYLE="background-color: #FFFFFF" VALUE="変更">
					</TD>
				</FORM>
				<FORM action="kanri_admin.jsp" method="POST">
					<TD><INPUT TYPE="hidden" NAME="gr_num"
						VALUE="<%=group[0][z]%>"> <INPUT TYPE="hidden"
						NAME="gr_name" VALUE="<%=group[1][z]%>"> <INPUT
						TYPE="hidden" NAME="action" VALUE="gr_del"> <INPUT
						TYPE="hidden" NAME="table_select" VALUE="gru"> <INPUT
						TYPE="hidden" NAME="sql_select" VALUE="del"> <INPUT
						TYPE="hidden" NAME="pass" VALUE="<%=inpPassword%>"> <INPUT
						TYPE="submit" STYLE="background-color: #FFFFFF" VALUE="削除">
					</TD>
				</FORM>
			</TR>
			<%
	...
	<CENTER>
		<TABLE STYLE="width: 100%">
			<TR>
				<TH bgcolor="#000099"><FONT color="#FFFFFF"><B>5.
							予定区分登録</B></FONT></TH>
			</TR>
		</TABLE>
		...
		<FORM action="kanri_admin.jsp" method="POST" name="formYotei">
			<TABLE>
				<TR>
					<TD>区分</TD>
					<TD><SELECT NAME="kubun" onChange="change_junban(this.value)"
						style="float: left">
							<OPTION VALUE="1">予定</OPTION>
							<OPTION VALUE="2">場所</OPTION>
					</SELECT> <SELECT NAME="y_junban" style="display: block">
							<%
							...
							%>
					</SELECT> <select name="b_junban" style="display: none">
							<%
				...
				<TR>
					<TD>内容</TD>
					<TD><INPUT TYPE="text" NAME="y_data" SIZE="30"></TD>
				</TR>
			</TABLE>
			<INPUT TYPE="hidden" NAME="table_select" VALUE="yotei"> <INPUT
				TYPE="hidden" NAME="sql_select" VALUE="ins"> <INPUT
				TYPE="hidden" NAME="action" VALUE="yotei_input"> <INPUT
				TYPE="hidden" NAME="pass" VALUE="<%=inpPassword%>"> <INPUT
				TYPE="submit" STYLE="background-color: #FFFFFF; width: 50"
				VALUE="登録">
		</FORM>
			...
			<TD valign="TOP">
				<TABLE border="1">
					...
					<TR STYLE="font-size: 12">
						...
						<FORM action="kanri_admin.jsp" method="POST">
							<TD align="center"><INPUT TYPE="hidden" NAME="y_kubun"
								VALUE="<%=yotei[0][t].trim()%>"> <INPUT TYPE="hidden"
								NAME="y_basyo" VALUE="<%=yotei[1][t]%>"> <INPUT
								TYPE="hidden" NAME="action" VALUE="yotei_dat_del"> <INPUT
								TYPE="hidden" NAME="table_select" VALUE="yotei"> <INPUT
								TYPE="hidden" NAME="sql_select" VALUE="del"> <INPUT
								TYPE="hidden" NAME="pass" VALUE="<%=inpPassword%>"> <INPUT
								TYPE="submit" STYLE="background-color: #FFFFFF" VALUE="削除">
							</TD>
			...
			<TD valign="TOP">
				<TABLE border="1">
					...
					<TR STYLE="font-size: 12">
						...
						<FORM action="kanri_admin.jsp" method="POST">
							<TD align="center"><INPUT TYPE="hidden" NAME="y_kubun"
								VALUE="<%=yotei[0][i].trim()%>"> <INPUT TYPE="hidden"
								NAME="y_basyo" VALUE="<%=yotei[1][i]%>"> <INPUT
								TYPE="hidden" NAME="action" VALUE="yotei_basyo_del"> <INPUT
								TYPE="hidden" NAME="table_select" VALUE="yotei"> <INPUT
								TYPE="hidden" NAME="sql_select" VALUE="del"> <INPUT
								TYPE="hidden" NAME="pass" VALUE="<%=inpPassword%>"> <INPUT
								TYPE="submit" STYLE="background-color: #FFFFFF" VALUE="削除">
