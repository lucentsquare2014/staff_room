...
<jsp:useBean id="ShainMST" scope="session"
	class="kkweb.beans.B_ShainMST" /><jsp:useBean id="Year_month"
	scope="session" class="kkweb.beans.B_Year_month" />
...
<body>
	<center>
		...
		<form method="post" action="s_shouninkakunin" name="C">
			<input TYPE="submit" VALUE="承認作業終了"
				STYLE="cursor: pointer; width: 100px; background-color: aqua"
				name="cc"> <input TYPE="hidden" NAME="iraisha_number"
				VALUE="<%=iraisha_number%>"> <input TYPE="hidden"
				NAME="iraisha_year_month" VALUE="<%=iraisha_year_month%>">
		</form>
		<br>
		<%
			}
		...
		<TABLE border="3" bordercolor="#008080" class="ichiran"
				...
				<%
					int m = 1;
						...
						for (int j = 0; j <= list.size() - 1; ++j) {
							...
							if (grouptbl.getGROUPnumber().equals("000")
									|| grouptbl.getGROUPnumber().equals("800"))
								continue;
							if (m > 5) {
				%>
			</TR>
			<TR>
				...
				%>
				<TD align="center" valign="top" class="ichiran" width="154"><FONT
					class="ichiran"><strong><%=grouptbl.getGROUPname()%></strong><br>
				</FONT> <%
 	//	String sql_2 = " where GROUPnumber='"+grouptbl.getGROUPnumber()+"' AND number !='"+ShainMST.getNumber()+"'";
 			String sql_2 = " where GROUPnumber='"
 					+ grouptbl.getGROUPnumber() + "'";
 			...
 			for (int i = 0; i <= slist.size() - 1; ++i) {
 				...
 				b++;
 				if (grouptbl.getGROUPnumber().equals(
 						bsyounin.getGROUPnumber())) {
 %>

					<FORM method="post" action="MailSend_Gamen4.jsp" name="A<%=b%>">
						<INPUT TYPE="submit" VALUE="<%=bsyounin.getName()%>"
							NAME="shouninsha_name<%=b%>" class="ichiran"
							onClick="submit1(<%=b%>)"> <INPUT TYPE="hidden"
							NAME="iraisha_number" VALUE="<%=iraisha_number%>"> <INPUT
							TYPE="hidden" NAME="iraisha_year_month"
							VALUE="<%=iraisha_year_month%>"> <INPUT TYPE="hidden"
							NAME="iraisha_mail" VALUE="<%=login_mail%>"> <INPUT
							TYPE="hidden" NAME="shouninsha_mail"
							VALUE="<%=bsyounin.getMail()%>"> <INPUT TYPE="hidden"
							NAME="shouninsha_name" VALUE="<%=bsyounin.getName()%>">
						<INPUT TYPE="hidden" NAME="shouninsha_number"
							VALUE="<%=bsyounin.getNumber()%>">
						<%
 ...
 %></TD>
		...
		<br> <a href="Menu_Gamen.jsp" class="link"><font class="link"><small>[
					メニューへ戻る ]</small></font></a>
	</center>
</body>
</html>
<%
