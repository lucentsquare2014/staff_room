...
<BODY BGCOLOR="#99A5FF">
	<TABLE BORDER="0">
		<TR>
			<TD><FORM ACTION="tryagain.jsp" METHOD="POST">
					<INPUT TYPE="hidden" NAME="id" VALUE="<%= ID %>"><INPUT
						TYPE="hidden" NAME="group" VALUE="<%= post %>"><INPUT
						TYPE="submit" VALUE="月表示" title="月表示に移動します。">
				</FORM></TD>
			<TD><FORM ACTION="TestExample34.jsp" METHOD="POST">
					<INPUT TYPE="hidden" NAME="id" VALUE="<%= ID %>"><INPUT
						TYPE="hidden" NAME="group" VALUE="<%= post %>"><INPUT
						TYPE="submit" VALUE="週表示" title="週表示に移動します。">
				</FORM></TD>
			<TD><FORM ACTION="h_hyoji.jsp" METHOD="POST">
					<INPUT TYPE="hidden" NAME="id" VALUE="<%= ID %>"><INPUT
						TYPE="hidden" NAME="group" VALUE="<%= post %>"><INPUT
						TYPE="submit" VALUE="日表示" title="日表示に移動します。">
				</FORM></TD>
			<TD><FORM ACTION="personal.jsp" METHOD="POST" TARGET="_parent">
					<INPUT TYPE="hidden" NAME="id" VALUE="<%= ID %>"><INPUT
						...
						<tr>
							...
							<form method="post" action="TestExample34.jsp">
								<td><input type="hidden" name="id" value="<%= ID %>">
									<input type="submit" name="previous" value="前の月"
									title="前の月に移動します。"> <input type="hidden" name="today"
									value="<%= bfrDay %>"> <input type="hidden"
									name="month" value="<%= bfrMonth %>"> <input
									type="hidden" name="year" value="<%= bfrYear %>"> <input
									type="hidden" name="group" value="<%= post %>"></td>
							</form>
							<%--  前の週へ  --%>
							<form method="post" action="TestExample34.jsp">
								<td><input type="hidden" name="id" value="<%= ID %>">
									<input type="submit" name="previous" value="前の週"
									title="前の週に移動します。"> <input type="hidden" name="today"
									value="<%= t %>"> <input type="hidden" name="month"
									value="<%= s %>"> <input type="hidden" name="year"
									value="<%= r %>"> <input type="hidden" name="group"
									value="<%= post %>"></td>
							</form>
							<td><SPAN CLASS="shadow"> <font size="6"
									color="white" title="日曜日が対象になっています。"> <b><i> <%
	// 今日の日付
...
%>
							...
							<form method="post" action="TestExample34.jsp">
								<td><input type="hidden" name="id" value="<%= ID %>">
									...
									<input type="hidden" name="today" value="<%= q %>"> <input
									type="hidden" name="month" value="<%= p %>"> <input
									type="hidden" name="year" value="<%= o %>"> <input
									type="hidden" name="group" value="<%= post %>"></td>
							</form>
							<%--  次の月へ　--%>
							<form method="post" action="TestExample34.jsp">
...
%>
					<%--　社員名表示用ボックス　--%>
					<tr>
						<td bgcolor="white"><font size="2"><%= kojNAME.elementAt(i) %></font>
						</td>
						<%
		// 週毎に表示
		c = f;
		for(j=0; j<7; j++){
			...
			%><A
							HREF="timeIn.jsp?id=<%= ID %>&no=<%= no %>&s_date=<%= sFmt.format(thisMonth) %>&s_start=&b_start=&group=<%= post %>&kind=Week&act="
							TARGET="sub02" title="新規登録できます。"
							oncontextmenu="if(!event.ctrlKey){ no='<%= no %>';date='<%= sFmt.format(thisMonth) %>'; miseru<%=xxx%>(no,date);return false}">
								<%
...
%> <%= sFmt.format(thisMonth) %>
			...
			while(BANA.next()){
				...
				if(b_start.equals(sFmt.format(thisMonth)) || hajime.equals(sFmt.format(thisMonth))){
					...
					%><a
										href="dayUp.jsp?id=<%= ID %>&no=<%= no %>&s_date=&s_start=&b_start=<%= b_start %>&group=<%= post %>&kind=Week-b&act="
										TARGET="sub02" title="<%= bmemo %>"> <font size=2>
												<%= bana + bmemo%>
										</font>
									</a>
									</td>
			...
			String S_eTimeH = s_end.substring(0,2);					//          「時」に分割
				...
				%> <a
				href="timeUp.jsp?id=<%= ID %>&no=<%= no %>&s_date=<%= s_date %>&s_start=<%= s_start %>&b_start=&group=<%= post %>&kind=Week-u&act="
				TARGET="sub02"
				oncontextmenu="if(!event.ctrlKey){id='<%= ID %>';no='<%= no %>';date='<%= s_date %>';start='<%= s_start %>'; send<%=mincnt%>(id,no,date,start);return false;}">
					<font size=2> <%= shyou1 + shyou2 + memo %>
				</font>
			</a>
				<%
			}
...
<!--
					...
					(no,date){
						...
						parent.main.location.href = "copyInsert.jsp?id="+ Eid +"&no="+ Eno +"&no2="+ charno +"&s_date="+ Es_date +"&s_date2="+ charde +"&s_start="+ Es_start +"&group=
				<%= post %>
					&kind=Week";
					}
