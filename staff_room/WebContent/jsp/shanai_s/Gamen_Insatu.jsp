...
<center>
<div class="noprint">
<font  class="title">勤務報告書aaaaaa</font>
<form>
<%	request.setCharacterEncoding("windows-31j");
	if (InsatuDATA == null || InsatuDATA.size() == 0 ){%>
<br><br>
<hr width="400">
<p><font color="#8B0000"><big>前画面に戻って選択し直してください</big></font></p>
<hr width="400">
<%	}else{
...
<center><br/><br/><A HREF="javascript:PrintPage()">印刷</A>
</center>
<div class="contents"><div align="left">＜＜承認経路＞＞</div><div align="left"><%= S1 %></div>
<center>
<%		}
...
<font size="4"><%= ym.substring( 0 , 4 ) %>年<%= mont.MonthCheck(ym.substring( 4 , 6 )) %>月 ： <%= year_month.getName() %></font><br>
...
<%=t %>年\<%=year_month.getName() %>\<%= year_month.getName()%>_<%=t %>年<%=m %>月.xls"><%= year_month.getName()%>_<%=t %>年<%=m %>月.xls--%>
</div><div class="noprint">
<table>
<tr>
<td><a href="Menu_Gamen.jsp" style="text-decoration:none;"><font class="link"><small>[ メニューへ戻る ]</small></font></a></td>
<td STYLE="cursor: default;"></td>
<%}session.removeAttribute("InsatuDATA");%>
<td><a href="Eturan_Gamen.jsp" style="text-decoration:none;"><font class="link"><small>[ 選択画面へ戻る ]</small></font></a></td>
</tr>
</table>
</body>
</html>
