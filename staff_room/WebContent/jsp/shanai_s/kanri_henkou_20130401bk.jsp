...
<%
	...
	String k_mail = "";
	if (table_select.equals("user")) {
		rs = stmt.executeQuery("select * from KOJIN where K_社員NO='"
				+ k_no + "'");
...
<BODY bgcolor="#F5F5F5">
	...
	<CENTER>
		<FORM action="kanri_admin.jsp" method="POST">
			<INPUT TYPE="hidden" NAME="action" VALUE="kojin_change"> <INPUT
				TYPE="hidden" NAME="name" VALUE="<%=k_name%>"> <INPUT
				TYPE="hidden" NAME="pass" VALUE="<%=k_pass%>"> <INPUT
				TYPE="hidden" NAME="new_num" VALUE="<%=k_no%>"> <INPUT
				TYPE="hidden" NAME="table_select" VALUE="user"> <INPUT
				TYPE="hidden" NAME="sql_select" VALUE="upd">
			<TABLE>
				<TR>
					<TD>承認者</TD>
					<%
					...
					<TD align="center" colspan="3">-１<INPUT TYPE="checkbox"
						NAME="ex1" <%=ex1_checked%>>- -２<INPUT TYPE="checkbox"
						NAME="ex2" <%=ex1_checked%>>-
					</TD>

				</TR>
				<TR>
					<TD>Ｉ Ｄ</TD>
					<TD><INPUT TYPE="text" NAME="new_id" SIZE="26"
						VALUE="<%=k_id%>"></TD>
					<TD>ﾒｰﾙｱﾄﾞﾚｽ</TD>
					<TD><INPUT TYPE="text" SIZE="30" NAME="new_mail"
						VALUE="<%=k_mail%>"></TD>
				</TR>
				<TR>
					<TD>ﾊﾟｽﾜｰﾄﾞ</TD>
					<TD><INPUT TYPE="password" NAME="new_pw" SIZE="20"
						VALUE="<%=k_pass%>"></TD>
					<TD>社員番号</TD>
				...
				<TR>
					<TD>氏 名</TD>
					<TD><INPUT TYPE="text" NAME="new_name" SIZE="26"
						VALUE="<%=k_name%>"></TD>
					<TD>所 属</TD>
					<TD align="left"><SELECT NAME="new_gr">
							<%
			...
			<TABLE>
				<TR>
					<TD><INPUT TYPE="submit" VALUE="　更新　"
						STYLE="background-color: #FFFFFF; width: 50"></TD>
					</FORM>
					<FORM action="kanri_admin.jsp" method="POST">
						<TD><INPUT TYPE="hidden" NAME="action" VALUE="admin">
...
<BODY bgcolor="#F5F5F5">
	...
	<CENTER>
		...
		<FORM action="kanri_admin.jsp" method="POST">
			<INPUT TYPE="hidden" NAME="action" VALUE="gr_change"> <INPUT
				TYPE="hidden" NAME="pass" VALUE="<%=kn_pass%>"> <INPUT
				TYPE="hidden" NAME="gr_num" VALUE="<%=gr_num%>"> <INPUT
				TYPE="hidden" NAME="gr_name" VALUE="<%=gr_name%>"> <INPUT
				TYPE="hidden" NAME="table_select" VALUE="gru"> <INPUT
				TYPE="hidden" NAME="sql_select" VALUE="upd">
			<TABLE>
				<TR>
					<TD>グループコード</TD>
					<TD><INPUT TYPE="text" NAME="n_gr_num" VALUE="<%=gr_num%>"
						SIZE="10"></TD>
				</TR>
				<TR>
					<TD>グループ名</TD>
					<TD><INPUT TYPE="text" NAME="n_gr_name" VALUE="<%=gr_name%>"
						SIZE="30"></TD>
				</TR>
			</TABLE>

			<INPUT TYPE="submit" STYLE="background-color: #FFFFFF; width: 50"
				VALUE="変更">
		</FORM>
